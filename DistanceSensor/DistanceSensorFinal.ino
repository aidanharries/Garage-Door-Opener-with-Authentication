...
void loop() {
  ...
  if ( ( CurrentMillis - OldMillis + 0xFFFF) % 0xFFFF >= SEND_TIME) {  //print every second
    ...
    jDoc["Unit"] = "Inches";
    if(distanceOutput <= 5) jDoc["Color"] = "https://upload.wikimedia.org/wikipedia/commons/b/b9/Solid_red.png";
    else if(distanceOutput >= 15) jDoc["Color"] = "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ec/Green_dark_square.jpg/220px-Green_dark_square.jpg";
    else jDoc["Color"] = "https://htmlcolorcodes.com/assets/images/colors/amber-color-solid-background-1920x1080.png";
    char output [256];
  ...
  CurrentMillis = millis() % 0xFFFF;
  if (( CurrentMillis - LEDMillis + 0xFFFF) % 0xFFFF >= distanceOutput * 100) {
